...
=== #link("https://www.luogu.com.cn/problem/P7560")[\[JOISC 2021\] フードコート]
...
$1<=N,M,Q<=2.5 times 10^5$，$K in [1,10^9]$，$B in [1,10^15]$。
]

看起来似乎这一道题目和整体二分并没有什么关系，不过若没有离开事件，则我们不妨可以二分*前多少个事件加入后（同时要求这些事件发生在白嫖事件前）*，队列里的元素数量大于 $B_i$，不妨设队列元素数量为 $x$，则若 $x>B_i$，说明能被赠送书虫的家庭加入的时间小于等于队列元素数量为 $x$ 的时间，否则大于，这样我们就可以用整体二分解决了。

若存在删除事件，容易发现我们可以对每个队列统计删了多少个，设其为 `del[i]`，则 `B[j]=del[A]+B[j]`。`del[i]` 可以用一个线段树通过合理的下传标记来维护。

若 $n,m,q$ 同阶，则时间复杂度 $cal(O)(n log^2 n)$。